...
namespace VNS.HIS.UI.THUOC
{
    public partial class frm_Themmoi_PhieuTrathuocThua : Form
    {
        ...
        void Initevents()
        {
            ...
            this.KeyDown += new KeyEventHandler(frm_Themmoi_PhieuTrathuocThua_KeyDown);
        ...
        bool isValidDetailData()
        {
            ...
            {
                Utility.SetMsg(lblMsg, "Bạn phải chọn thuốc trả", true);
                txtthuoc.Focus();
                return false;
            }
            if (Utility.DecimaltoDbnull(txtSoluongdutru.Text, -1) <= 0)
            {
                Utility.SetMsg(lblMsg, "Bạn phải nhập số lượng trả", true);
        ...
        void txtthuoc__OnSelectionChanged()
        {
            ...
            {
                Utility.SetMsg(lblMsg, cmdAddDetail.Enabled ? "" : "Chú ý: Thuốc bạn chọn không có trong " + cboKhoNhan.Text + " Đề nghị chọn lại thuốc!", false);
            }
        }
        ...
        private void AutocompleteThuoc()
        {
            ...
            try
            {
                DataTable _dataThuoc = SPs.ThuocLaythuoctrongkhoxuatAutocomplete(Utility.Int32Dbnull(cboKhoNhan.SelectedValue, -1), KIEU_THUOC_VT).GetDataSet().Tables[0];// new Select().From(DmucThuoc.Schema).Where(DmucThuoc.KieuThuocvattuColumn).IsEqualTo(KIEU_THUOC_VT).And(DmucThuoc.TrangThaiColumn).IsEqualTo(1).ExecuteDataSet().Tables[0];
                if (_dataThuoc == null)
                {
        ...
        private void getThuocTrongKho()
        {
            ...
            {
                m_dtDataThuocKho =
            SPs.ThuocLayDanhsachthuoctrongkho(Utility.Int32Dbnull(cboKhoNhan.SelectedValue),
                                             KIEU_THUOC_VT).GetDataSet().Tables[0];
                ...
                Utility.SetDataSourceForDataGridEx_Basic(grdKhoXuat, m_dtDataThuocKho, true, true, "1=1", "");
            }
            catch (Exception)
            {
            }
        ...
        {
            TPhieuNhapxuatthuoc objPhieuNhap = Taophieutrathuocthua();
        ...
        private void AddDetails()
        {
            ...
            {
                Utility.ShowMsg("Lỗi khi trả thuốc:\n" + ex.Message);
            }
        }
